...
//#define use_shadows
    ...
    void main(void)
    {
       ...
       #ifdef flipTexture
          vec4 extraColor  = texture2D(textureS3o2, vec2(gl_TexCoord[0].s,1-gl_TexCoord[0].t));
       #else
          vec4 extraColor  = texture2D(textureS3o2, gl_TexCoord[0].st);
       #endif
       ...
       #ifdef flipTexture
          gl_FragColor     = texture2D(textureS3o1, vec2(gl_TexCoord[0].s,1-gl_TexCoord[0].t));
       #else
       ...
       #else
            gl_FragColor.rgb = mix(teamColor.rgb, gl_FragColor.rgb, extraColor.a); // teamcolor
            gl_FragColor.rgb = gl_FragColor.rgb * reflection + specular;
       ...
       //gl_FragColor.rgb = mix(gl_FragColor.rgb, nightColor, 0.3); //night
       gl_FragColor.rgb = gl_FragColor.rgb * nightColor;
